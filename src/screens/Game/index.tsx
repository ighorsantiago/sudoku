...
import { Modal, FlatList, ActivityIndicator, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
export function Game() {
    ...
    const { level } = route.params as RouteParams;
    // const level = 'easy';
    ...
    async function handleBoard(game: string[]) {
        ...
        for (let i = 0; i < 9; i++) {
            for (let j = 0; j < 9; j++) {
                ...
                // let blockColor = (block % 2) === 1 ? "#504d5e" : "#3a3944";
                let blockColor = (block % 2) === 1 ? "white" : "lightgray";
    ...
    function handleUndo() {
        ...
        if (lastCellChanged.length >= 1) {
            const lastCell = {...lastCellChanged[lastCellChanged.length - 1], backColor: highlight};
            const id = lastCell.index;

            const boardUpdated = board.map(cell => {
                if (cell.index === id) {
    ...
    return (
        ...
        <Container>
            <Header>
                ...
                <Title>SUDOKU</Title>
                <Settings onPress={() => setShowModal(true)}>
                    <MaterialIcons
                        name="settings"
                        size={30}
                        color="#FFF"
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <Cell
                                ...
                                selected={selectedCell.index === item.index}
                                color={item.color}
                ...
                <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                    <ModalView>
                        <ModalButton onPress={() => setShowModal(false)}>
                            <ModalText style={{ fontSize: 25, color: 'red' }}>
                                X
                            </ModalText>
                        </ModalButton>
                        <ModalText>
